import { calculateDewPoint, convertPressure, getCurrentDateTime, getWindDirection, getWeatherForecastData } from "./utils.js";
...
export const renderWidgetForecast = (widget, data) => {
    ...
    const items = forecastData.map((item) => {
        ...
        widgetDayItem.insertAdjacentHTML('beforeend', `
            ...
            <p class="widget__day-temp">${Math.round(item.minTemp - 273.15)}°/${Math.round(item.maxTemp  - 273.15)}°</p>
        `);
